...
//https://blog.csdn.net/qq_25458977/article/details/116648270
...
#include "GridCalibrationTargetAprilgrid.hpp"
...
int board_width = 6;//标定板上的行数
int board_height = 6;//标定板上的列数
std::vector < cv::Point2f > circle_grid_centers;//用于记录检测到的圆的中心点（一张图片）
double circle_distance = 0.003f;

std::vector < std::vector <cv::Point3f> > object_points;//圆点中心的世界坐标,每张图片上的点构成内层vector，所有图片构成外层vector
std::vector < std::vector <cv::Point2f> > image_points;//圆点中心的图像坐标,,每张图片上的点构成内层vector，所有图片构成外层vector
...
int  main()
{
	...
	aslam::cameras::GridCalibrationTargetAprilgrid aprilgrid( board_height, board_width, 0.04561, 0.3 );
	VideoCapture cap;
	cap.open(2); //打开摄像头
	if(!cap.isOpened())
		return -1;
	...
	while (true) {
		cap >> x;
		cv::Mat frame = x.clone();
		cv::cvtColor(frame, frame, cv::COLOR_RGB2GRAY);
		cv::imshow("ds",frame);
		std::vector< cv::Point2f > points2ds;
		std::vector< cv::Point3f > points3ds;
		std::vector< bool > out_corner_observed;
		bool succ =  aprilgrid.computeObservation( frame, points2ds, out_corner_observed );
		points3ds = aprilgrid.points3d( );
		if (!succ) continue;
		std::vector< cv::Point2f > points2_fin;
		std::vector< cv::Point3f > points3_fin;
		for ( int idx_p = 0; idx_p < out_corner_observed.size( ); idx_p++ )
		{
			if ( out_corner_observed.at( idx_p ) )
			{
				// std::cout
				// <<" " <<points2ds.at( idx_p ).x
				// <<"  " << points2ds.at( idx_p ).y
				//
				// << " " <<points3ds.at( idx_p ).x
				// << " " <<points3ds.at( idx_p ).y
				// << " " <<points3ds.at( idx_p ).z<<std::endl;
		...
		if (waitKey(50) == 27) //Esc键退出，ESC的ASCLL码为27
		{
			if (succ) {
				...
				image_points.push_back(points2_fin);
				object_points.push_back(points3_fin);
				if (image_points.size() > 20) {
					...
					cv::fisheye::calibrate(object_points, image_points, frame.size(),
					                       camera_matrix,
					                       distort_matrix,
					                       cam_r_vec, cam_t_vec,
					                       flags,
...
int main3()
{
	...
	while (true) {
		...
		found_circles = cv::findChessboardCorners(frame, board_size, circle_grid_centers,cv::CALIB_CB_SYMMETRIC_GRID);
		// // std::cout << "foundCircles:  " << foundCircles << std::endl;
		if (found_circles) {
			if (circle_grid_centers.empty()) continue;
		// 	cv::cornerSubPix(frame, circle_grid_centers, cv::Size(11, 11), cv::Size(-1, -1),
		// 	                 TermCriteria(TermCriteria::EPS | TermCriteria::MAX_ITER, 30, 0.1));
			cv::Mat out_pic;
			cv::cvtColor(frame, out_pic, cv::COLOR_GRAY2BGR);
			cv::drawChessboardCorners(out_pic, board_size, circle_grid_centers, found_circles);
			cv::imshow("检测到的点", out_pic);
			cv::waitKey(2);
		}

...
#include "camera_model/apriltag/TagFamily.h"
TagFamily *tag36h11 = new TagFamily(tagCodes36h11);

// available tag families:

#include "camera_model/apriltag/Tag16h5.h"
#include "camera_model/apriltag/Tag16h5_other.h"
#include "camera_model/apriltag/Tag25h7.h"
#include "camera_model/apriltag/Tag25h9.h"
#include "camera_model/apriltag/Tag36h11.h"
#include "camera_model/apriltag/Tag36h11_other.h"
#include "camera_model/apriltag/Tag36h9.h"
...
void
TagFamily::setErrorRecoveryBits( int b )
{
    errorRecoveryBits = b;
}

void
TagFamily::setErrorRecoveryFraction( float v )
{
    errorRecoveryBits = ( int )( ( ( int )( minimumHammingDistance - 1 ) / 2 ) * v );
}

unsigned long long
TagFamily::rotate90( unsigned long long w, int d )
{
...
int
TagFamily::hammingDistance( unsigned long long a, unsigned long long b )
{
    return popCount( a ^ b );
}

unsigned char
TagFamily::popCountReal( unsigned long long w )
{
    unsigned char cnt = 0;
    while ( w != 0 )
    {
...
int
TagFamily::popCount( unsigned long long w )
{
    int count = 0;
    while ( w != 0 )
    {
...
void
TagFamily::decode( TagDetection& det, unsigned long long rCode ) const
{
    int bestId                  = -1;
...
void
TagFamily::printHammingDistances( ) const
{

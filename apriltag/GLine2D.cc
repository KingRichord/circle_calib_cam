...
float
GLine2D::getLineCoordinate( const std::pair< float, float >& pt )
{
    normalizeSlope( );
...
GLine2D::getPointOfCoordinate( float coord )
{
    normalizeP( );
...
std::pair< float, float >
GLine2D::intersectionWith( const GLine2D& line ) const
{
    float m00 = dx;
...
GLine2D
GLine2D::lsqFitXYW( const std::vector< XYWeight >& xyweights )
{

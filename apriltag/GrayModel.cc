...
void
GrayModel::addObservation( float x, float y, float gray )
{
    float xy = x * y;
...
float
GrayModel::interpolate( float x, float y )
{
    if ( dirty )
        compute( );
...
GrayModel::compute( )
{
    // we really only need 4 linearly independent observations to fit our answer, but we'll
    // be very sensitive to noise if we don't have an over-determined system. Thus, require
    // at least 6 observations (or we'll use a constant model below).

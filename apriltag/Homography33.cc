...
#ifdef STABLE_H
void
Homography33::setCorrespondences( const std::vector< std::pair< float, float > >& sPts,
...
#else
void
Homography33::addCorrespondence( float worldx, float worldy, float imagex, float imagey )
{
    valid = false;
    imagex -= cxy.first;
    imagey -= cxy.second;
...
Homography33::project( float worldx, float worldy )
{
    compute( );
